...
if (-Not (Test-Path ".\pokémon_xx.gba"))
{
    Write-Host "File 'pokémon_xx.gba' not found."
    Write-Host $help
    exit
}
...
&$objdump -D -bbinary -marmv4t -Mforce-thumb --start-address="$start_str" --stop-address="$end_str" .\pokémon_xx.gba > .\pokémon_xx.dump
Invoke-Expression "$DiffTool .\baserom.dump .\pokémon_xx.dump"
